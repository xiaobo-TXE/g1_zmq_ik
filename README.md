...
不想让末端跟着标记转，就设 `aruco.send_quat = false`（或命令行 `--no-quat`）：帧里只发 `{"pos": ...}`，
**末端姿态保持"启动瞬间锁定的朝向"** —— 所以启动 `main.py` 时手臂要处于你想要的进给姿态（手朝前、手指水平）。

水平进给抓薄盒子时，盒子被推走通常不是"进给太深"，而是这两件事之一：

1. **接近时夹爪是闭合的** —— 启动时若没给夹爪目标，机器人侧会保持它自己的状态（可能就是闭合的），
   手臂带着闭合的手指往前推 ✗。做法：`main` 段写 `"grip": 100`（启动即张开到 100%），或跑起来先敲 `go`。
2. **开合方向没跨在 3cm 窄面上** —— 用 `axes(ee/torso)` 的 y 确认它跨的是盒子窄面；若跨的是 13cm 宽面，
   两指会撞进盒子（改 rpy 解决不了，得把盒子原地转 90°）。

真的要**少伸一点**时，用 `--marker-to-grasp` 的**面内分量**把抓取点往回收：偏移是在标签自身坐标系里给的，
本仓库实测的那张贴法下 `+tag.y` 指向机器人，所以 `0 0.02 -0.09` = 比原来少伸 2cm。
核对方法：`--no-send` 时打印的 `抓取位姿 torso p=` 与 `[TARGET] torso p=` 之差，就是三个分量的实际效果。

> 备选（对"立着的薄盒子"最不容易推）：**从上往下抓** —— `--grasp-align-rpy 0 1.5708 0`（探入方向竖直向下、
> 手指左右），`--marker-to-grasp 0 0 -0.03`（盒口往下 3cm），夹爪从正上方下来，不存在"往前顶"的问题。
...
> **关键：标签贴哪儿决定进深**。水平进给要求"两指夹持中心"到达目标点，而目标点在标记正下方：
> 标签贴在顶面**中间**时目标点落在盒体内部（半深），手指比需要的进深短，手会先把盒子顶走；
> 标签贴在顶面**靠机器人那一端**时，目标点就在盒子近端，手指只需跨过近侧面即可 —— 这也是本仓库
> 现在采用的贴法。若标签仍贴在中间而想保持水平进给，就用 `marker_to_grasp` 的**面内分量**把目标点
> 往机器人方向拉回（`+tag.y` 朝机器人，例如 `0 0.045 -0.09` 拉回 4.5cm），或者改成从上往下抓
> （`0 1.5708 1.5708` + `0 0 -0.03`）。
```
...
### 2.6.1 笛卡尔直线段（抓取进给 / 退出）

**问题**：控制器默认是"每周期都用同一个最终目标做一次完整反解"，于是末端走的是**关节空间
最短路径划出来的弧**，不是直线。实测（`tools/audit_motion_path.py`）：87mm 的移动离直线
10.7mm，119mm 的移动离直线 **139.9mm** —— 抓取时"最后几厘米"如果歪着进去，手指就会撞到
盒子或把盒子推走。

**做法**：只在**最后一段**走直线（工业上就是 PTP + LIN 的用法，长距离转移仍走关节空间）：

```
① PTP(关节空间)  当前位姿 ──▶ pre-grasp        ← 怎么绕都行，快、不易撞
② LIN(笛卡尔直线) pre-grasp ──▶ 抓取点          ← 沿**工具轴**进给，位置直线 + 姿态轴角插值
③ 闭爪（--grip-on-arrive / --grip-on-arrive-soft）
④ LIN             抓取点 ──▶ 退出点             ← 沿工具轴反向拉出来（--lin-retreat）
```

```bash
# 进给 120mm（先到 pre-grasp 再直线进给），闭爪完成后退出 100mm
python main.py --config robot.json --lin-approach 120 --lin-retreat 100 --lin-jerk 2.0
```

- `--lin-approach MM`：**0 = 关闭（默认，行为与旧版完全一致）**。开了之后，收到 6003 的
  **绝对位置**目标（或启动 `--pos`）就先到"沿**工具轴 x** 后退 MM 的 pre-grasp 点"，
  到位后再直线进给到目标。**6003 协议一个字都不用改**：`send_quat=false` 时主程序握着
  启动锁定的末端朝向，而工具轴就是进给方向，所以位置指令不带姿态也能算对方向。
  `delta` 增量帧**不触发**接近（那是连续轨迹流，插进去就毁了）。
- `--lin-retreat MM`：到位 + 闭爪完成后沿工具轴反向直线退出 MM（0=不动）。
- `--lin-speed / --lin-accel / --lin-jerk / --lin-rot-speed`：直线段自己的限幅，
  默认沿用 `--ee-speed / --ee-accel / --ee-jerk / --ee-rot-speed`。
- `--lin-abort-cycles N`：直线段里反解连续失败 N 个周期就**取消该段并停住**（默认 5=100ms），
  绝不带着发散的解放着走。

**时间律**：位置直线 + 姿态沿测地线 `R(s)=R0·exp(s·log(R0ᵀR1))`（等价 SLERP，无万向锁）；
进度 s(t) 是一条规划好的 **S 形/梯形**曲线（速度 + 加速度 + jerk 三重限幅，终点速度精确为 0）。

**两条安全性质**（`tools/test_cartesian_lin.py` 逐条验证）：

1. **路点绝不跑在手臂前面**：反解失败 -> 路点回退（冻结）；关节被限速 -> 直线段退一拍
   （`直线段同步减速`）。所以"末端偏离直线"这件事在两大约束下都不会发生。
2. **到位判定看的是终点，不是路点**：`self.target` 始终是终点（`info.ee_target` 全程等于终点），
   路点只喂给 IK。所以直线段走到一半**不会**被误判"到位"，走完并停稳 dwell 后才判。

日志里能看到进度：`[直线: 42.3% 剩 69.2mm (0.76/1.80s)]`。
...
│   ├── test_cartesian_lin.py 笛卡尔直线段单测（直线度/姿态插值/限幅/IK失效即停/到位判据，37 项）
