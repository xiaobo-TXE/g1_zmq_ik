...
离线单测（不需要机器人、不需要 URDF）：`python tools/test_arrival.py` —— 覆盖到位判据、驻留、滞回、路过、不可达、被挡住、状态超时、目标变更等 40 项检查。
...
```
[   1.35s] #60   0.2ms meas=(+0.350,-0.200,+0.100) tgt=(+0.350,-0.200,+0.100) ik_err=0.1mm/0.0° track_err=0.1mm/0.0° ik=0.2ms 到位=✅(0.1mm, 1.29s)
```
...
│   ├── test_arrival.py     到位判定离线单测（合成 StepInfo 序列，40 项检查）
