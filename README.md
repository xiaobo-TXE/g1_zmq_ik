...
三档限速，从"想要的速度"到"兜底安全"依次是：

| 参数 | 单位 | 说明 |
|---|---|---|
| `--ee-speed` | m/s | **末端笛卡尔速度上限**，例如 `0.05` = 5cm/s。用雅可比把本周期关节增量按比例缩放，所以与 IK 权重/求解器无关，**实测就是设定值**（0.05 → 50.0mm/s） |
| `--ee-rot-speed` | rad/s | 末端姿态角速度上限，例如 `0.5` |
| `--max-step-deg` | ° | 每个关节每周期的最大增量（关节侧兜底），默认 2°，50Hz 下约 100°/s |

```bash
# 匀速 5cm/s 移到目标
python main.py --robot-ip <IP> --arm right --pos 0.35 -0.20 0.10 --ee-speed 0.05

# 配合目标流：外部想发多快都行，实际末端被压在 8cm/s
python main.py --robot-ip <IP> --arm right --ee-speed 0.08
python tools/send_target.py --mode circle --rate 30 --radius 0.05 --period 4
```

日志里可以直接核对：`v= 50.0mm/s` 是**本周期指令末端的实际速度**，`末端限速 x0.14` 是钳制系数
（<1 表示被限速了）。**不给 `--ee-speed` 时不做笛卡尔限速**，手臂会以 IK 与关节限位允许的速度尽快到达。

> 注意：`--ee-speed` 只限速度、不限加速度，所以是"匀速逼近 + 到位即停"。若需要 S 形加减速（起停更柔和），
> 告诉我，我再加 `--ee-accel`。
