...
三档运动限制，从"想要的速度"到"兜底安全"依次是：

| 参数 | 单位 | 说明 |
|---|---|---|
| `--ee-speed` | m/s | **末端笛卡尔速度上限**，如 `0.05` = 5cm/s |
| `--ee-rot-speed` | rad/s | 末端姿态角速度上限，如 `0.5` |
| `--ee-accel` | m/s² | **末端加速度上限**，如 `0.2`（0→5cm/s 用时 0.25s） |
| `--ee-rot-accel` | rad/s² | 末端姿态角加速度上限 |
| `--max-step-deg` | ° | 每个关节每周期的最大增量（关节侧兜底），默认 2° |

实现方式是**用雅可比把本周期的关节增量整体缩放**（`v = J·dq`，再按 `上限×dt/|v|` 缩放），
所以**与 IK 用什么权重、哪个求解器无关，设定值一定成立**，也不会去改 IK 的目标。

给了 `--ee-accel` 会得到一条**梯形速度曲线**：按加速度爬升 → 匀速巡航 → 临近目标按 `√(2·a·剩余距离)`
提前减速，因此"起停柔和、且到位不过冲"。

```bash
# 8cm/s 上限、0.2m/s² 加速度（约 0.4s 加到最高速）
python main.py --robot-ip <IP> --arm right --pos 0.35 -0.20 0.10 --ee-speed 0.08 --ee-accel 0.2

# 配合目标流：外部想发多快都行，实际末端被压在设定值内
python main.py --robot-ip <IP> --arm right --ee-speed 0.08 --ee-accel 0.2
python tools/send_target.py --mode circle --rate 30 --radius 0.05 --period 4
```

日志里可以直接核对（`v` 是本周期指令末端的实际速度、`a` 是实际加速度、`末端限速 xN` 是钳制系数）：

```
[  0.42s] #20 ... v=  76.2mm/s a= 0.20m/s² 关节限速 6 个; 末端限速 x0.11
[  1.30s] #60 ... v=  80.0mm/s a= 0.00m/s² 关节限速 2 个; 末端限速 x0.23
[  2.64s] #120 ... v=   8.8mm/s                                     ← 临近目标自动减速
```

实测（假机器人 + 真 ZMQ 链路）：`--ee-speed 0.05` → 稳定 50.0mm/s；`--ee-accel 0.2` → 实测加速度
稳定 0.200m/s²，曲线为 加速 0.4s → 巡航 80.0mm/s → 提前减速到位，无过冲。

> 不给 `--ee-speed/--ee-accel` 时不做笛卡尔限制，手臂以 IK 与关节限位允许的速度尽快到达。
...
| `--ee-speed` | `0` | 末端笛卡尔速度上限 m/s（如 `0.05`=5cm/s）；`0`=不限 |
| `--ee-rot-speed` | `0` | 末端姿态角速度上限 rad/s；`0`=不限 |
| `--ee-accel` | `0` | 末端加速度上限 m/s²（如 `0.2`）；`0`=不限 → 梯形速度曲线 |
| `--ee-rot-accel` | `0` | 末端姿态角加速度上限 rad/s²；`0`=不限 |
