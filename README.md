...
- **松爪后自动归位**（`place_return = true`，默认开）：先沿工具轴直线退出
  `place_return_retreat`（默认 100mm）离开盒子，再按 Z→Y→X 轴分解直线段走回**启动这一程序时的
  末端位姿**（`main.py` 启动瞬间记下的实测位姿）。所以：**启动前先把手摆到你希望它回到的位置**。
  归位途中若有新的抓取目标进来，会放弃归位直接去抓（日志会说明）。想让它松爪后停在放置点，
  用 `--no-place-return` / `place_return = false`。
...
`--place-return`/`--no-place-return`（默认**开**：松爪后归位）、`--place-return-retreat MM`（100，
归位第一步沿工具轴退出的距离）。
