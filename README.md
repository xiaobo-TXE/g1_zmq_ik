...
|---|---|---|
| 盒子**顶面**、面朝上 | `z≈(0,0,+1)` | 想夹盒子腰部（高 3cm）→ `0 0 -0.015` |
| 盒子**正面**、面朝相机/人 | `z≈` 水平指向你 | 想夹盒子中间（进深 3cm）→ `0 0 -0.015` |
...
| `0 0 90` | 沿标记 x（水平） | 沿标记 x | 水平探入，但开合方向转 90° |
| `0 90 0` | **−标记 z（竖直向下）** | 沿标记 y | 从上往下夹 |
| `0 90 90` | −标记 z（竖直向下） | 沿标记 x | 从上往下夹，开合方向转 90° |
...
| **`0 90 0`** | (0.57,0.59,−0.58) 前-左-下 | ✅ 到位 4.48s，残差 0.07mm/0.03° |
| `0 0 90` | (0.58,0.44,0.68) 前-左-上 | ✗ 不可达（147mm/30.5°） |
| `0 90 90` | (0.36,−0.05,−0.93) 竖直向下 | `ik_err` 4.4mm（可解，正在爬过去） |
